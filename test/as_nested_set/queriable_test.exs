defmodule AsNestedSet.QueriableTest do
  ...
  def create_tree(taxonomy_id) do
    ...
    n010 =
      insert(:taxon, name: "n010", lft: 4, rgt: 5, parent_id: n01.id, taxonomy_id: taxonomy_id)

    n011 =
      insert(:taxon, name: "n011", lft: 6, rgt: 7, parent_id: n01.id, taxonomy_id: taxonomy_id)
  ...
  test "children/2 should get children filtered by the query" do
    {root, [{no_child, []}, _]} = create_tree(1)

    assert match(
             from(t in Taxon, where: t.name != "n01")
             |> children(root)
             |> execute(),
             [%{name: "n00", lft: 1, rgt: 2, taxonomy_id: 1}]
           )
  ...
  test "descendants/2 returns the decendants filtered by the query" do
    {root, _} = create_tree(1)

    assert match(from(t in Taxon, where: t.name != "n010") |> descendants(root) |> execute(), [
             %{name: "n00", lft: 1, rgt: 2, taxonomy_id: 1},
             %{name: "n01", lft: 3, rgt: 8, taxonomy_id: 1},
             %{name: "n011", lft: 6, rgt: 7, taxonomy_id: 1}
           ])
  ...
  test "root/2 returns the root of the tree" do
    ...
    assert match(
             root(Taxon, %{taxonomy_id: 1}) |> execute(),
  ...
  test "self_and_descendants/1 returns self and all descendants" do
  ...
  test "self_and_descendants/2 returns self and all descendants filtered by the query" do
    {_, [_, {target, _} | _]} = create_tree(1)

    assert match(
             from(t in Taxon, where: t.name != "n010")
  ...
  test "self_and_siblings/1 should return self and all its siblings" do
  ...
  test "self_and_siblings/2 should return self and siblings filtered by the query" do
    {_, [{target, _} | _]} = create_tree(1)
